...
def _st_image_compat(image, **kwargs):
    """Call st.image with a fallback for older/newer Streamlit APIs.

    Some Streamlit releases expect `use_container_width`, others use
    `use_column_width`. Try the preferred kwarg and fall back if needed.
    """
    try:
        return st.image(image, **kwargs)
    except TypeError:
        # map use_container_width -> use_column_width for older versions
        kw = kwargs.copy()
        if 'use_container_width' in kw:
            kw.pop('use_container_width')
            kw['use_column_width'] = True
        try:
            return st.image(image, **kw)
        except TypeError:
            # last resort: call without optional kwargs
            return st.image(image)


def _st_button_compat(label, **kwargs):
    """Call st.button while removing unsupported kwargs on older Streamlit."""
    try:
        return st.button(label, **kwargs)
    except TypeError:
        kw = {k: v for k, v in kwargs.items() if k != 'use_container_width'}
        try:
...
def main():
    ...
    image_path = Path('C:/Users/Thinkpad T460s/Downloads/mlop tb/MLOPS TUBES/WhatsApp Image 2025-12-15 at 16.31.00_1c0a35a3.jpg')
    ...
    if image:
        _st_image_compat(image, use_container_width=True)
    else:
    ...
    with st.container():
        ...
        if _st_button_compat('Diagnosa Lahan', use_container_width=True):
